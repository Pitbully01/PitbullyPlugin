...
class TpAcceptCommandTest {

    @Test
    void nonPlayer() {
        CommandSender sender = mock(CommandSender.class);
        boolean handled = new TpAcceptCommand().onCommand(sender, mock(Command.class), "tpaccept", new String[]{});
    ...
    void noPermission() {
        ...
        boolean handled = new TpAcceptCommand().onCommand(p, mock(Command.class), "tpaccept", new String[]{});
        assertThat(handled).isTrue();
    ...
    void acceptsAndNotifies() {
        ...
        try (MockedStatic<TpaRequestManager> mgr = mockStatic(TpaRequestManager.class)) {
            ...
            boolean handled = new TpAcceptCommand().onCommand(p, mock(Command.class), "tpaccept", new String[]{});
            assertThat(handled).isTrue();
            verify(req).acceptRequest();
            verify(p).sendMessage(contains("angenommen"));
        }
    }
}
